...
class DataStore:
    ...
    def record_decision(self, decision, snapshot_id: int) -> int:
        d = decision.to_dict()
        cur = self.conn.execute(
            """INSERT INTO decisions
               (snapshot_id, ts, symbol, direction, confidence, risk_reward,
                market_condition, entry, stop, tp1, tp2, votes, reasons)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    def record_trade(self, symbol: str, side: str, opened_ts: str,
        ...
        self.conn.execute(
            """INSERT INTO trades
               (decision_id, symbol, side, opened_ts, closed_ts, pnl, exit_reason)
               VALUES (?,?,?,?,?,?,?)""",
    ...
    # ---- readers (research) ----
    def executed_trades_with_votes(self) -> list[dict]:
        """Closed trades joined to the decision that opened them."""
        rows = self.conn.execute(
            """SELECT t.pnl, t.side, t.symbol, t.closed_ts, d.votes, d.confidence
        ...
        return [
            {"pnl": r[0], "side": r[1], "symbol": r[2], "closed_ts": r[3],
             "votes": json.loads(r[4]), "confidence": r[5]}
            for r in rows
        ]
